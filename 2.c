...
void work(char *Folder)
{
    ...
    for (int shunxv = 0; shunxv < jishuqi; shunxv++)
    {
        ...
        if (l)
        {
            ...
            // 如果pw为NULL，显示UID；gr为NULL显示GID
            printf("%s %s ", 
                   pw ? pw->pw_name : uid_to_str(st.st_uid),
                   gr ? gr->gr_name : gid_to_str(st.st_gid));
    ...
    if (R)
    {
        ...
        {
            char *filename = entries[Shu].name;
            // 跳过当前目录（.）和上级目录（..），避免循环
            if (strcmp(filename, ".") == 0 || strcmp(filename, "..") == 0)
            {
                continue;
            }
            // 拼接完整路径（避免路径长度溢出，snprintf返回值要检查）
            int ret = snprintf(buf, PATH_MAX, "%s/%s", Folder, filename);
