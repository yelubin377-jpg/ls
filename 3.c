...
void work(char *Folder)
{
    ...
    struct WenJianMessage entries[1024];//1024 MAX
    int jishuqi = 0;//  文件技术器
    ...
    while ((ent = readdir(dir)) != NULL)// NULL是暗号，读完了，即没读完的时候疯狂循环
    {
        ...
        strcpy(entries[jishuqi].name, ent->d_name);//复制文件名到entries数组
    ...
    if (R)
    {
        ...
        {
            if (strcmp(entries[Shu].name, ".") == 0 || strcmp(entries[Shu].name, "..") == 0)//康康是不是隐藏文件
            {
                continue;//不跳过：当前目录 → .. → 上级 → .. → 再上级 → ……  无限循环，程序好像有点4了
            }

            snprintf(buf, PATH_MAX, "%s/%s", Folder, entries[Shu].name);///
            // 使用lstat检查，避免跟随符号链接
            struct stat lst;
            if (lstat(buf, &lst) == 0 && S_ISDIR(lst.st_mode))
            {
                // 如果是符号链接，跳过以避免无限递归
                ...
                {
                    printf("\n%s:\n", buf);
                    work(buf);//递归调用整个work函数，无限拆套娃，直到没德拆1
                }
            }
        }
    }
